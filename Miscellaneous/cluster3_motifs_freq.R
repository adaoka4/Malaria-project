...
dat=processFile("allupreg8_cluster3pfpvp_filtered_251021.txt")
...
speciesfqdf1=data.frame(x=rep(uni1,2),y=speciesfq1,variable=rep(species, each = 3))
...
speciesfqdf2=data.frame(x=rep(uni2,2),y=speciesfq2,variable=rep(species, each = 3))
...
speciesfqdf3=data.frame(x=rep(uni3,2),y=speciesfq3,variable=rep(species, each = 4))
...
plist <- list(p1,p2,p3)
motifplots <- grid.arrange(grobs = plist, ncol = 3, top = "Distribution of Cluster3 motif seeds")
